...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
  ...
  <!-- 
    This file contains just the constraints for the profile PatientRead
  ...
  <sch:pattern>
    ...
    <sch:rule context="f:AuditEvent/f:agent">
      ...
      <sch:assert test="count(f:network[x]) &gt;= 1">network[x]: minimum cardinality of 'network[x]' is 1</sch:assert>
      <sch:assert test="count(f:type) &gt;= 1">type: minimum cardinality of 'type' is 1</sch:assert>
      <sch:assert test="count(f:network[x]) &lt;= 0">network[x]: maximum cardinality of 'network[x]' is 0</sch:assert>
    </sch:rule>
  </sch:pattern>
  <sch:pattern>
